...
- 新索引失败：系统保留上一active索引；若其成员已与有效发布版本不一致，检索回退到当前有效发布内容的词法检索。检查 `/api/search/status` 的 `index_consistent` 后重建索引。索引重建成功不等于全库通过最终验收。
...
镜像包含 LibreOffice、`python3-uno` 和中文字体。PDF 转换先刷新真实目录及页码，再输出 PDF；并发转换使用独立配置目录。DOCX 单独下载保留可更新目录，最终页码以实际排版为准。宿主机仅安装 LibreOffice 可执行文件不足以运行本项目的 UNO 转换流程，建议使用 Docker 部署。

全库收口保留发布重审、正式知识覆盖、索引一致性、检索评测和法律事项等独立门槛。历史最后任务失败时，恢复后会显示暂停与具体失败阶段；不要直接改数据库将运行置为完成。被撤回的已接受知识需重新修订、独立裁决并通过发布复核，旧版本仍保持撤回。
