...
在项目工作台中先填写“项目资料”，再解析要求、核对条款分类、生成目录和章节。条款按技术、资格、商务、合同、参考和待分类展示，保留原文、编号、页码和分类理由。系统的非技术建议须由实际复核人确认，才能移出正式技术覆盖分母；资格、商务和合同条款还要填写实际响应或不适用依据。

已有草稿时禁止重新解析原文；允许补充目录映射，保留原章节正文及历史版本，相关旧签审失效。“处理待办”区分生成补写、正文响应、证据核验、实际资料和条款范围，分别统计已映射、已回应、已签审。实际正文已有响应时，可以逐字定位并绑定，再重新签审；招标要求本身不算技术响应。

长章节按要求分段生成并保存过程记录。模型失败或漏回应的分段可单独修复，完成的分段保留原文。修复前后核对项目资料、条款映射、当前正文和知识发布版本；并发修改或取消时不覆盖原稿。旧稿没有有效分段记录、人工修改过正文或引用已失效时，工作台会说明需要重新编写该章或人工补录。

项目事实会核对完整招标原文及来源页，检查对象、数字、单位、范围和前提条件。项目资料或核验规则变化后必须重新检查；重查保留正文和历史人审事件，不自动签审。修改、弱化或删除表述须实际编辑正文，不能仅把提示改成“已处理”。
...
本轮改进与验证记录见 [条款与补写工作流交接](docs/handoffs/WORKFLOW_IMPROVEMENTS_20260908.md)。
