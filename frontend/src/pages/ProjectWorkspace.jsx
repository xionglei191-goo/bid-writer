...
function Editor({ project, reload, setMessage }) {
  const [selectedId, setSelectedId] = useState(project.sections[0]?.id || null); const [content, setContent] = useState(""); const [busy, setBusy] = useState(false); const [reviewer, setReviewer] = useState("技术负责人"); const [resolutions, setResolutions] = useState([]);
  ...
  async function confirm() { if (!selected?.draft) return; setBusy(true); try { await post(`/api/projects/drafts/${selected.draft.id}/confirm`, { reviewer, resolutions: resolutions.map((resolution, index) => ({ index, resolution })) }); await reload(); setMessage("本章已逐项处理待确认事项，并绑定当前正文完成签审。"); } catch (error) { setMessage(error.message); } finally { setBusy(false); } }
...
function EvidencePanel({ claims, onChanged, setMessage }) {
  ...
  async function resolve(action) {
    ...
    try { await post(`/api/projects/claims/${selected.id}/resolve`, { action, resolution }); await onChanged(); setMessage("Claim处理结论已记录。"); }
...
function ProjectQuality({ project }) {
  ...
  async function exportFile(format) { setBusy(true); try { const queued = await post(`/api/projects/${project.id}/export`, { format, background: true }); const result = await waitForJob(queued, { onProgress: (job) => setMessage(`${job.stage} · ${job.progress}%`) }); setMessage(`交付已冻结：${result.manifest_hash}`); } catch (error) { setMessage(error.message); } finally { setBusy(false); } }
  useEffect(() => { check(); }, [project.id]);
  return <section className="panel"><div className="panel-title"><div><ShieldCheck size={18} /><h2>正式交付门禁</h2></div><div className="actions"><BusyButton busy={busy} className="secondary" onClick={check}><CheckCheck size={16} />重新检查</BusyButton><BusyButton busy={busy} className="secondary" onClick={() => exportFile("docx")} disabled={!report?.ready}><Download size={16} />DOCX</BusyButton><BusyButton busy={busy} onClick={() => exportFile("package")} disabled={!report?.ready}><Download size={16} />正式交付包</BusyButton></div></div>{message && <Notice>{message}</Notice>}{report && <><div className="metric-grid four"><Metric label="质量分" value={report.score} tone={report.ready ? "success" : "warning"} /><Metric label="章节草稿" value={`${report.metrics.drafts}/${report.metrics.sections}`} /><Metric label="条款覆盖" value={`${report.metrics.coverage}%`} /><Metric label="证据支持" value={`${report.metrics.claims.support_rate}%`} tone={report.metrics.claims.high_unsupported ? "danger" : "success"} /></div><div className="gate-result"><div className={report.ready ? "gate-badge ready" : "gate-badge blocked"}>{report.ready ? "可正式交付" : "尚未通过门禁"}</div>{report.blockers.map((item) => <article key={item.key}><strong>{item.title}</strong><span>{item.detail}</span></article>)}{report.warnings?.map((item) => <article key={`${item.key}-${item.detail}`}><strong>{item.title}</strong><span>{item.detail}</span></article>)}</div></>}</section>;
}
